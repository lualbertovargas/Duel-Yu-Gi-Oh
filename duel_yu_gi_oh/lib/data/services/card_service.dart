...
class CardService {
  ...
  Future<List<Card>> getCardsByArchetype(String archetype) async {
    final response = await http.get(Uri.parse('$baseUrl/cardinfo.php?archetype=$archetype'));

    if (response.statusCode == 200) {
      final Map<String, dynamic> jsonResponse = json.decode(response.body);
      final List<dynamic> data = jsonResponse['data'];
      return data.map((json) => _cardFromJson(json)).where((card) => !card.isBanned).toList();
    } else {
